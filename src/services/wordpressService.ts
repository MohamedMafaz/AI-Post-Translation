...
export const fetchPosts = async (credentials: WordPressCredentials): Promise<Post[]> => {
  ...
  try {
    ...
    const response = await fetch(`${formattedUrl}/wp-json/wp/v2/posts?per_page=100&page=1&order=desc&orderby=date`, {
      method: 'GET',
      headers: {
        'Authorization': 'Basic ' + btoa(`${username}:${appPassword}`)
      },
...
export const publishTranslatedPost = async (
  ...
  try {
    ...
    const createResponse = await fetch(`${formattedUrl}/wp-json/wp/v2/posts?lang=${language}`, {
      method: 'POST',
      headers: {
        'Authorization': 'Basic ' + btoa(`${username}:${appPassword}`),
